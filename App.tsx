          ...
          {payload.map((entry: any, index: number) => {
             ...
             return (
              ...
              >
                {/* Best Value Badge */}
                {isCheapest && (
                  <div className="absolute -top-3 -right-2 bg-black text-white text-[10px] font-display uppercase px-2 py-1 transform rotate-2 shadow-sm border border-white">
                    ★ BEST PRICE
                  </div>
                )}
                ...
                <div className="flex items-center justify-between gap-4">
                   ...
                   <span className={`font-display font-bold whitespace-nowrap ${isCheapest ? 'text-xl' : 'text-sm'}`}>
                     £{unitPrice.toFixed(2)}
                   </span>
                </div>
                ...
                {confirmedAmount > 0 && (
                   <div className={`mt-2 pt-1 border-t ${isCheapest ? 'border-black border-dashed' : 'border-gray-200'} flex justify-between items-center`}>
                      ...
                      <span className={`font-display ${isCheapest ? 'text-lg' : 'text-sm'}`}>
                        £{total.toFixed(2)}
                      </span>
                   </div>
                )}
...
const App: React.FC = () => {
  ...
  const TimeSelectors = (
    <div className="flex items-center gap-2 bg-white/50 p-1 border-2 border-transparent hover:border-black transition-colors rounded-sm">
      ...
      <span className="text-lg font-black font-display">→</span>
      <select 
        value={yearEnd} 
  ...
  return (
    <div className="min-h-screen bg-pop-cyan/20 flex flex-col font-body">
      {/* Hidden File Input */}
      <input 
        type="file" 
        ref={fileInputRef} 
...
/>
      ...
      {/* Main Content */}
      <main className="flex-grow max-w-7xl mx-auto w-full p-6 md:p-12 z-10">
        
        {/* 2. SELECT SOURCES (Legend) */}
        <div className="mb-8">
          <div className="bg-white border-4 border-black p-6 shadow-hard-lg">
             <div className="flex justify-between items-end mb-6">
                ...
                <p className="text-xs font-body font-bold text-gray-400 uppercase tracking-widest hidden md:block">Toggle to filter master chart</p>
             </div>
           ...
           <div className="relative z-10 bg-white border-4 border-black p-8 shadow-hard flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex-1 w-full">
                ...
                <input 
                  ...
                  placeholder="e.g. 100"
                  className="w-full bg-gray-100 border-4 border-black p-4 font-body font-black text-4xl focus:bg-pop-cyan/20 focus:outline-none transition-colors"
                />
              </div>
              ...
              {confirmedAmount > 0 && (
                 <div className="hidden md:flex flex-col items-center justify-center p-4 border-4 border-black border-dashed bg-pop-pink/10 h-full rotate-2">
                    <span className="font-body font-bold text-xs">CALCULATING FOR</span>
        ...
        {/* 4. MARKET MASTERPIECE (Main Chart) */}
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={unifiedData} margin={{ top: 20, right: 20, bottom: 20, left: 0 }}>
                    ...
                    <XAxis 
                      ...
                      tick={{ fill: 'black', fontWeight: 700, fontFamily: 'Inter' }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mt-2">
            {activeCountries.map(country => analyses[country.id] && (
              <div key={country.id} className="bg-white border-4 border-black p-4 shadow-hard-sm transform hover:-translate-y-1 transition-transform">
                ...
                <p className="font-body font-bold text-md leading-tight">"{analyses[country.id]}"</p>
                <p className="text-right text-xs font-black mt-2 text-gray-500 font-display">- GHOST OF ANDY</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <section className="mt-12 mb-12">
          <div className="border-b-8 border-black mb-8 pb-2">
            <h2 className="text-4xl md:text-6xl font-display uppercase leading-none tracking-tighter text-black drop-shadow-sm">
              Executive Strategy <span className="text-pop-magenta">Dashboard</span>
            </h2>
          </div>

          {/* New 2-column Grid Layout to align Cards with Charts */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            
            {/* COLUMN 1: Stability / Trend */}
            <div className="flex flex-col gap-6">
              {/* Hedge Card - Yellow for Colombia */}
              <div className="bg-pop-yellow border-4 border-black p-8 shadow-hard transform rotate-1 hover:rotate-0 transition-transform duration-300">
                <div className="bg-black text-white inline-block px-3 py-1 font-display text-lg mb-4 transform -rotate-1">
                  RISK MANAGEMENT
                </div>
                <h3 className="font-display text-5xl mb-4 leading-none uppercase">Stability King</h3>
                <p className="font-body text-xl font-bold leading-snug border-l-4 border-black pl-4">
                  Although <span className="bg-white px-2 border-2 border-black mx-1 inline-block transform skew-x-3">GUATEMALA</span> currently has the lowest price, it has the highest price growth rate, meaning its price advantage is gradually shrinking. Conversely, <span className="bg-white px-2 border-2 border-black mx-1 inline-block transform -skew-x-3">COLOMBIA</span> has the flattest slope, indicating the most mature price system, minimal volatility, and the strongest resistance to inflation.
                </p>
              </div>
              
              {/* Retro Trend Chart */}
              <div className="bg-white border-4 border-black p-4 shadow-hard h-full">
                  <h4 className="font-display text-2xl uppercase mb-4 border-b-4 border-black pb-2 bg-gray-100 -mx-4 -mt-4 px-4 pt-4">
                    Price Trends: Which Country is Rising Fastest?
                  </h4>
                  <div className="h-[300px] bg-white border-4 border-black p-2">
                     <RetroTrendChart />
                  </div>
              </div>
            </div>

            {/* COLUMN 2: Leader / Win Rate */}
            <div className="flex flex-col gap-6">
              {/* Leader Card - Magenta for Guatemala */}
              <div className="bg-pop-magenta border-4 border-black p-8 shadow-hard transform -rotate-1 hover:rotate-0 transition-transform duration-300">
                <div className="bg-black text-white inline-block px-3 py-1 font-display text-lg mb-4 transform rotate-1">
                  STRATEGIC INSIGHT
                </div>
                <h3 className="font-display text-5xl mb-4 leading-none uppercase">2026 Price Leader</h3>
                <p className="font-body text-xl font-bold leading-snug border-l-4 border-black pl-4">
                  Based on forecast trends, <span className="bg-white px-2 border-2 border-black mx-1 inline-block transform -skew-x-6">GUATEMALA</span> is projected to offer the lowest absolute price 100% of the time in 2026.
                </p>
              </div>

               {/* Retro Win Rate Chart */}
               <div className="bg-white border-4 border-black p-4 shadow-hard h-full">
                  <h4 className="font-display text-2xl uppercase mb-4 border-b-4 border-black pb-2 bg-gray-100 -mx-4 -mt-4 px-4 pt-4">
                    Percentage of months where the country had the lowest market price
                  </h4>
                  <div className="h-[300px] bg-white border-4 border-black p-2">
                     <RetroWinRateChart />
                  </div>
